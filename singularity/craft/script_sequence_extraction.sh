...
#EAS
if [ x"${SKIP_CLEAR_SEQ_EXTRACT}" == "x" ]; then 
	#echo "Value is not assigned to a variable"
	echo "SKIP_CLEAR_SEQ_EXTRACT is not set, so proceed to delete temporary files." ; 
	rm annotation*
	...
	echo "SKIP_CLEAR_SEQ_EXTRACT is set so do NOT delete temporary files." ; 
