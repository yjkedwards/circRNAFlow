...
class Source(object):
    ...
    def pull_model(self, model):
        print(f"Want to pull model {model}")
...
def printTB():
    print("START  SHOW TRACEBACK FOR DEBUG")
    traceback.print_stack(f=None, limit=None, file=None)
    print("FINISH SHOW TRACEBACK FOR DEBUG")
...
class GithubPermalinkSource(Source):
    ...
    def _parse_url(cls, url):
        ...
        if "/tree/" not in url_dir:
            raise ValueError("'/tree/' missing in the url {0}. Typical github format " +
                             "is localhost/<user>/<repo>/tree/<commit>/<directory>")
    ...
    def get_lfs_source(self, component):
        ...
        remote_url = "https://localhost/{user}/{repo}.git".format(user=user, repo=repo)
